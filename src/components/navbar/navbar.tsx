...
const SearchBar: React.FC = () => {
  ...
  const dummySearchResults = [
    { id: 1, title: "React Basics", mentor: "John Doe", topic: "React" },
    {
      id: 2,
          ...
          {searchQuery && (
            <>
              <div className="absolute z-10 w-full mt-1 bg-white rounded-md shadow-lg">
                {dummySearchResults
                  .filter(
                    (item: { title: string; mentor: string; topic: string }) =>
                      item.title
                        .toLowerCase()
      ...
      const getNotifications = async () => {
        try {
          const notificationsResp = await fetchNotifications(
            get_cookie(document.cookie, "access_token")
          );
          console.log(
            "🚀 ~ getNotifications ~ notificationsResp:",
            notificationsResp
          );
