    ...
    <div className="max-w-6xl mx-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
      {sessions.map((session) => (
        ...
        >
          ...
          />
          <h3 className="font-bold text-amber-500 line-clamp-2">{session.title}</h3>
          <p className="text-sm font-bold text-amber-400 line-clamp-2">
            {session.short_description}
          </p>
          <p className="text-bold text-amber-500 mt-4">{session.schedule}</p>
        </div>
      ))}
    </div>
  </div>
