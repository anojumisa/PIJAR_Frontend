...
> => {
  try {
    ...
    const response = await privateService.get<NotificationsResponse>(
      `/users/notifications`
    );
    return response;
  } catch (error) {
...
export const fetchLearnersInterests =
  async (): Promise<LearnersInterestResponse> => {
    try {
      const response = await privateService.get<LearnersInterestResponse>(
        `/learners/interests`
      );
      return response.data;
    } catch (error) {
...
export const fetchSessionClass = async (sessionId: number) => {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to fetch session class"
    );
  }
};
...
export const toggleFollowMentor = async (
  ...
  try {
    // if (!mentorId) {
    // 	throw new Error("Mentor ID is undefined");
    // }
...
export const getMentorFollowStatus = async (mentorId: number) => {
  try {
    console.log("Mentor ID:", mentorId);
